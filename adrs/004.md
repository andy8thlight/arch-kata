...
- **Fleet distribution and rebalancing** (routing, ADR-0005)  
- **Pricing and promotion strategies** (ADR-0003)  
...
- All forecasts and explanations are stored in the **Data Platform (ADR-0009)** for retraining and audit.
...
| **AI Interchange Layer** | Provides orchestration, cost tracking, and governance hooks. | Shared AI infra (ADR-0016, ADR-0020, ADR-0019). |
| **Feature Store (ADR-0018)** | Stores input features for training/inference. | Refreshed hourly from telemetry. |
| **Data Platform (ADR-0009)** | Manages pipelines, data lineage, and model logs. | Integration with IoT, weather, and event feeds. |
| **Routing Service (ADR-0005)** | Consumes forecasts for route planning and pre-positioning. | Subscribes to forecast topics. |
| **Pricing Engine (ADR-0003)** | Adjusts rates based on predicted demand. | Uses hourly or daily forecasts. |
...
flowchart LR
    ...
    subgraph Consumers
        ROUTE[Routing Service (ADR-0005)]
        PRICE[Pricing Engine (ADR-0003)]
...
| AI cost overruns                       | Low–Med    | Med    | Cost guardrails via AI Interchange (ADR-0020).           |
