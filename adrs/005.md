...
**Trade-offs**
- Increased orchestration complexity (batch + streaming). Batch and streaming pipelines must coordinate solver state handoff and resolve conflicts (e.g., if a real-time update overlaps with a batch plan, the most recent event takes precedence and triggers a state reconciliation process).
- Requires careful solver tuning and state synchronisation.
- Additional monitoring and rollback mechanisms for failed updates. Monitoring is implemented via health checks, latency/error dashboards, and alerting. Rollback is handled by reverting to the last known good solver state and reprocessing missed events.
- The architecture is designed for future extensibility to distributed/edge optimisation: solver components can be deployed closer to the field (e.g., at regional hubs) to reduce latency and improve resilience, with central coordination for global constraints and data sharing.
...
| **Vector Database (optional)** | Store route embeddings for similarity / explainability | Reuses embedding pipeline from ADR-009 |
    - The Vector Database is queried when additional context or explainability is needed, such as retrieving routes that succeeded under similar historical conditions. It is used on-demand for explainability or to enhance routing decisions in ambiguous scenarios. Fallback to standard optimisation occurs if no similar routes are found.
| **Audit Log** | Record optimisation decisions, inputs, and versions | Immutable; supports SLA traceability |
    - Audit logs are retained for a minimum of 7 years to meet compliance requirements. Access is restricted to authorised personnel and all access is logged for audit purposes.

### Optimisation API Example

Sample request:
```json
{
    "requests": [
        {"pickup": "HUB-1", "dropoff": "HUB-2", "window": ["09:00", "10:00"]}
    ],
    "vehicles": [
        {"id": "VAN-1", "capacity": 10, "shift": ["08:00", "16:00"]}
    ],
...
{
    "routes": [
        {"vehicle": "VAN-1", "stops": ["HUB-1", "HUB-2"], "eta": ["09:15", "09:45"]}
    ],
    "total_distance_km": 25.4,
    "explanation": "Selected based on time window and minimal distance."
}
```
...
sequenceDiagram
    ...
    Note over SOLV,VDB: Optional route similarity lookup (on-demand for context/explainability)
