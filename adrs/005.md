...
- How do we incorporate **AI demand forecasts** (ADR-004) and hub capacity predictions?
...
- Incorporates **demand forecasts** (ADR-004) to prioritise tasks and pre-positioning, and **hub-slot reservation** (ADR-010) for valid destinations.
...
- Logs every solution (inputs, seed, objective, constraints, plan) to **Audit Log** (ADR-011) and **Data Platform** (ADR-007).

> **AI-assist (optional, phased):** enable an **ML re-ranking module** to reorder solver-tied solutions (choose best among k-opt neighbourhoods) when measurable lift ≥ X% and within cost guardrails (ADR-018). The deterministic solver stays the source of truth.
...
**Inputs**
- Forecasts per hub/time, hub capacity & slots
...
| Integration failures (event bus/API) | Low–Med | Med | Idempotent events; retries & DLQ (ADR-001); reconciliation jobs |
...
### Links
* ADR-001 – External Dependency SLA & Retry Handling
